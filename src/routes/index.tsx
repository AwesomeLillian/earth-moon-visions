    ...
    services: [
      {
        ...
        points: ["CAT6 & fibre trenching","Data point termination","Rack & patch panel builds","Switch & power installs","Testing & certification"],
        images: [
          { url: serverRackFull.url, alt: "Floor-to-ceiling server cabinet" },
          { url: patchPanels.url, alt: "Full server rack with patch panels" },
          { url: rackSwitch.url, alt: "Rack-mounted switch" },
          { url: dataPoint.url, alt: "Wall-mounted data point" },
          { url: trenching.url, alt: "Team excavating trench" },
          { url: compaction.url, alt: "Crew compacting soil" },
          { url: chamber.url, alt: "Cable chamber" },
        ],
      },
      {
        title: "Computer Peripherals",
        body: "Supply and installation of computers, monitors, keyboards, routers, switches, enclosures and everyday IT hardware.",
        points: ["Hardware supply", "On-site installation", "Ongoing support"],
        images: [{ url: peripherals.url, alt: "Office desk setup" }],
      },
    ],
  },
  {
    ...
    services: [
      {
        ...
        fit: "contain",
        imageHeight: "tall",
        body: "Genuine HP, Samsung and compatible toners and cartridges.",
        points: ["HP & Samsung stock", "Bulk orders", "Fast delivery"],
        images: [{ url: tonerHp.url, alt: "HP toners" },{ url: tonerSamsung.url, alt: "Samsung toner" }],
      },
      {
        title: "Media Printing",
        body: "Banners, signage, posters, flyers and large-format media printing.",
        points: ["Banners & signage", "Flyers & posters", "Large format"],
        images: [{ url: mediaPrinting.url, alt: "Large format printing" }],
      },
      {
        title: "Embroidery Printing",
        fit: "contain",
        body: "Corporate branding on golf shirts, overalls, caps and PPE.",
        points: ["Corporate workwear", "Logo embroidery", "PPE branding"],
        images: [{ url: embroidery.url, alt: "Embroidery" }],
      },
    ],
  },
  {
    ...
    services: [
      {
        ...
        fit: "contain",
        imageHeight: "tall",
        body: "Paving, kerbing and site finishing for driveways, streets and property entrances.",
        points: ["Driveways & walkways", "Street paving", "Kerbing & edging"],
        images: [{ url: paving.url, alt: "Paving" },{ url: pavingRoad.url, alt: "Paved road" }],
      },
    ],
  },
];
...
function ContactForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em) || em.length > 255) return setError("Please enter a valid email address.");
    if (!msg || msg.length > 1000) return setError("Please enter a message (max 1000 characters).");
    setError(""); setSending(true); setStatus("Sending...");
    try {
      const res = await fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: n, email: em, message: msg }) });
      const data = await res.json();
      if (res.ok) { setStatus("Message sent successfully! ✅"); setName(""); setEmail(""); setMessage(""); }
      else { setStatus(""); setError(data.error || "Failed to send, try again"); }
    } catch { setStatus(""); setError("Failed to send, try again. Check internet."); }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur">
        <div className="mx-auto flex w-full max-w- items-center justify-between gap-4 px-5 py-2.5 lg:px-10 xl:px-16">
        ...
        {menuOpen && (<nav id="main-menu" className="border-t border-border/60 bg-background/95 backdrop-blur"><div className="mx-auto flex w-full max-w- flex-col px-5 py-2 lg:px-10 xl:px-16">{navLinks.map((l) => (<a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="rounded-lg px-2 py-3 text-sm text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">{l.label}</a>))}</div></nav>)}
      </header>

      <main>
        {/* HERO - was max-w-, now full width */}
        <section className="surface-glow relative overflow-hidden border-b border-border/60">
          <div className="mx-auto grid w-full max-w- gap-12 px-5 py-16 md:min-h- md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] md:items-center md:gap-10 md:py-20 lg:gap-16 lg:px-10 xl:px-16 lg:py-24">
            <div className="min-w-0 md:py-4">
              ...
              <div className="mt-8 flex flex-wrap gap-3"><a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">About Us<ArrowUpRight className="h-4 w-4" /></a><a href="#services" className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-semibold text-foreground transition-colors hover:bg-secondary">Our services<ArrowUpRight className="h-4 w-4" /></a></div>
        ...
        {/* SERVICES - was max-w-6xl, now full width */}
        <section id="services" className="mx-auto w-full max-w- px-5 py-20 lg:px-10 xl:px-16"><h2 className="text-3xl font-bold md:text-4xl">OUR FULL SERVICES</h2><p className="mt-3 max-w-2xl text-muted-foreground">Five service lines, one accountable team — from the trench outside to the branded shirt on your staff.</p><div className="mt-12 space-y-16">{serviceGroups.map((g) => (<div key={g.group}><div className="flex items-center gap-4"><h3 className="text-xs font-semibold uppercase tracking-[0.3em] text-primary">{g.group}</h3><span className="h-px flex-1 bg-border/70" /></div><div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{g.services.map((s) => (<ServiceCard key={s.title} service={s} />))}</div></div>))}</div></section>

        {/* ABOUT - was max-w-6xl, now full width */}
        <section id="about" className="border-y border-border/60 bg-card/40"><div className="mx-auto grid w-full max-w- gap-10 px-5 py-20 md:grid-cols-2 md:items-center lg:px-10 xl:px-16"><div className="overflow-hidden rounded-3xl border border-border/70"><img src={trenching.url} alt="Team excavating" loading="lazy" className="h-full w-full object-cover" /></div><div><h2 className="text-3xl font-bold md:text-4xl">About the company</h2><p className="mt-5 text-muted-foreground">We have grown from consumable supply into a full-service infrastructure and branding partner.</p><p className="mt-4 text-muted-foreground">Whether it is a single toner delivery or a full server room and network rollout, the same team stands behind the work.</p><a href="#contact" className="mt-8 inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-semibold transition-colors hover:bg-secondary">Get in touch<ArrowUpRight className="h-4 w-4" /></a></div></div></section>

        {/* CONTACT - was max-w-6xl, now full width */}
        <section id="contact" className="mx-auto w-full max-w- px-5 py-20 lg:px-10 xl:px-16"><h2 className="text-3xl font-bold md:text-4xl">Visit or Contact Us</h2><div className="mt-10 grid gap-8 md:grid-cols-2"><div className="space-y-5"><div className="flex gap-3 rounded-2xl border border-border/70 bg-card p-5"><MapPin className="mt-0.5 h-5 w-5 shrink-0 text-primary" /><div><p className="font-semibold">Office</p><p className="mt-1 text-sm text-muted-foreground">{ADDRESS}</p><a href={MAP_LINK} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">Open in Google Maps<ArrowUpRight className="h-3.5 w-3.5" /></a></div></div><div className="overflow-hidden rounded-2xl border border-border/70"><iframe title="Map" src={MAP_EMBED} loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="h- w-full border-0" /></div></div><ContactForm /></div></section>
      </main>

      <footer className="border-t border-border/60 bg-card/40"><div className="mx-auto flex w-full max-w- flex-col gap-6 px-5 py-10 md:flex-row md:items-center md:justify-between lg:px-10 xl:px-16"><div className="h-20 w- max-w-full overflow-hidden"><img src={logo.url} alt="Moon And Earth Trading logo" className="h-full w-full object-contain object-left" /></div><div className="text-sm text-muted-foreground"><p>{ADDRESS}</p><p className="mt-1">© 2009 Moon And Earth Trading And Projects 11cc. All rights reserved.</p></div></div></footer>
    </div>
  );
}
...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Moon And Earth Trading And Projects 11cc" },
      { property: "og:description", content: "Network points, server rooms, peripherals, toners, media printing and embroidery across South Africa." },
